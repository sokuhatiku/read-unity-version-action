...
test('not exists project', () => {
    expect(() => UnityVersionDescribedFile.ExploreSync('__tests__/UnityProject_NotExistsProject'))
        .toThrowError();
})

test('invalid format project', () => {
    expect(() => UnityVersionDescribedFile.ExploreSync('__tests__/UnityProject_InvalidVersionFormat'))
        .toThrowError();
})